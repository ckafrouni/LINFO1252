...
void print_all_blocks()
{
    ...
    while ((uint8_t *)current < MY_HEAP + sizeof(MY_HEAP))
    {
        ...
        printf("\033[31;1m#%d\033[0m\t", num_block);
        if (is_allocated)
        {
            printf("\033[33;1mallocated\toffset = %u\t%%p = %p\tsize = %u\n\033[0m", offset, current, size);
        }
        else
        {
            uint16_t next_offset = *(current + 1);
            uint16_t prev_offset = *(current + 2);
            printf("\033[32;1mfree\t\toffset = %u\t%%p = %p\tsize = %u\tnext_offset = %u\tprev_offset = %u\n\033[0m", offset, current, size, next_offset, prev_offset);
        }
